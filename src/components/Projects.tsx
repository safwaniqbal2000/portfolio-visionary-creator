...
const Projects = () => {
  ...
  const filteredProjects = filter === 'all' ? projectsData : projectsData.filter(project => project.category.includes(filter));
  return <section id="projects" ref={ref as React.RefObject<HTMLDivElement>} className="py-24 px-4 md:px-8">
      <div className="container mx-auto max-w-7xl">
        ...
        <div className={`flex justify-center flex-wrap gap-2 mb-12 ${isInView ? ANIMATION_CLASSES.slideUp : 'opacity-0'} animation-delay-200`}>
          {projectCategories.map(category => <button key={category.id} onClick={() => setFilter(category.id)} className={cn("px-4 py-2 text-sm font-medium rounded-full transition-all", filter === category.id ? "bg-primary text-primary-foreground shadow-md shadow-primary/10" : "bg-secondary text-secondary-foreground hover:bg-secondary/70")}>
              {category.label}
            </button>)}
        </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ${isInView ? ANIMATION_CLASSES.slideUp : 'opacity-0'} animation-delay-300`}>
          {filteredProjects.map(project => <a key={project.id} href={project.link} className="group flex flex-col bg-card rounded-xl overflow-hidden border border-border/50 hover:border-primary/20 shadow-sm hover:shadow-lg transition-all duration-300">
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.skills.slice(0, 3).map((skill, skillIndex) => <span key={skillIndex} className="px-3 py-1 bg-secondary text-secondary-foreground text-xs font-medium rounded-full">
                      {skill}
                    </span>)}
                  {project.skills.length > 3 && <span className="px-3 py-1 bg-secondary text-secondary-foreground text-xs font-medium rounded-full">
